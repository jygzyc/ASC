...
在 565 MiB 的微信 `base.apk` 上的 11 个场景中，rasc 相对 ASC 的几何平均加速比为 **4.8×**，
且各场景结果集与参考实现一致；小体积归档差距更大（9.8 MiB 的 Settings APK 上 `classes` 为 31×），
而 `getclass` 只有约 1.2-2.7×，因为双方都要先解析整个归档。
